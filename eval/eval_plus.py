...
def main(args):
    ...
    with distributed_state.main_process_first():
        ...
        if args.model == 'controlnet':
            ...
            # DreamBooth model - can be fine-tuned SD model
            try:
                # Try loading as a DiffusionPipeline first (more general)
                ...
                # Fallback to StableDiffusionPipeline
                pipe = StableDiffusionPipeline.from_pretrained(
                    args.model_path,
                    safety_checker=None,
                    torch_dtype=torch.float16
                )
        elif args.model == 'dreambooth-controlnet':
            # DreamBooth + ControlNet combination
            controlnet = ControlNetModel.from_pretrained(args.controlnet_path, torch_dtype=torch.float16)
            pipe = StableDiffusionControlNetPipeline.from_pretrained(
                pretrained_model_name_or_path=args.model_path,  # DreamBooth model path
                controlnet=controlnet,
                safety_checker=None,
                torch_dtype=torch.float16
            )
    ...
    if args.model in ['controlnet', 'dreambooth-controlnet']:
        if args.ddim:
    ...
    with distributed_state.split_between_processes(list(range(len(dataset)))) as local_idxs:
        ...
        for idx in local_idxs:
            ...
            # Handle condition for control-based models
            condition = None
            if args.model not in ['dreambooth']:
            ...
            # Process conditions for control-based models
            ...
            # Prepare conditions for batch processing (only for control-based models)
            if condition is not None:
                condition = condition.resize((args.resolution, args.resolution), Image.Resampling.NEAREST)
                conditions = [condition] * args.batch_size
            else:
                conditions = None

            # Generate images based on model type
            if args.model == 'dreambooth':
                # Pure DreamBooth - text-to-image only
                images = pipe(
                    prompt=prompts,
            ...
            elif args.model == 't2i-adapter-sdxl' and args.task_name == 'lineart':
                images = pipe(
                    prompt=prompts,
                    image=conditions,
                    num_inference_steps=args.num_inference_steps,
                ...
                # Control-based models (ControlNet, T2I-Adapter, DreamBooth+ControlNet)
                images = pipe(
                    prompt=prompts,
                    image=conditions,
                    num_inference_steps=args.num_inference_steps,
            ...
            # Process and save task-specific outputs
            if args.task_name == 'canny':
            ...
            if args.model == 'dreambooth':
                # For pure DreamBooth: just original image and generated images
                images = [image] + images
                images = [F.pil_to_tensor(x) for x in images]
                images = make_grid(images, nrow=len(images))
            elif args.task_name in ['canny', 'canny_cv2']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--controlnet_path', type=str, default=None, help='ControlNet path for DreamBooth+ControlNet combination')
